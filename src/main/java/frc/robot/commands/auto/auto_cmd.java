...
public class auto_cmd extends SequentialCommandGroup {
  ...
  public auto_cmd() {
    ...
    addCommands(
        ...
        new ConditionalCommand(new auto_cmd_terminal(), new WaitCommand(0),
            () -> RobotContainer.RC().driverControls.readSideboard(SBButton.Sw14)),
        new InstantCommand(RobotContainer.RC().drivetrain::printPose),
        new InstantCommand(() -> {
          RobotContainer.RC().sensors.setAutoEndPose(RobotContainer.RC().drivetrain.getPose());
        }),
        new InstantCommand(RobotContainer.RC().limelight::disableLED));
  }
