//command to drive X, Y based on xbox joystick, but heading always to centerfield
...
public class SwerveDriveCenterCommand extends CommandBase {
  ...
  public SwerveDriveCenterCommand(SwerveDrivetrain drivetrain, DriverControls dc) {
  ...
  public void execute() {
    ...
    //set rotation velocit as output from PID, feeding in current POS heading in degrees
    final var rot = rotLimiter.calculate(anglePid.calculate(drivetrain.getPose().getRotation().getDegrees())) * DriveTrain.kMaxAngularSpeed;
